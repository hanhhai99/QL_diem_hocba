...
namespace QLdiem_hocba.Chucnang
{
    public partial class Themtk : Form
    {
        ...
        public Themtk()
        {
            ...
            DataTable data = nhanvien.lay_lop();
            for(int i = 0; i < data.Rows.Count; i++)
            {
                cblop.Items.Add(data.Rows[i][0].ToString());
            }
        ...
        private void tao_nam(int ngay, int thang,ComboBox x)
        {
            ...
            {
                for (int i = now.Year - 25; i <= now.Year; i++)
                {
                    x.Items.Add("" + i);
                }
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (nhanvien.kiemtra_mataikhoan(tbma.Text) == 0)
            {
                ...
                nhanvien.Tao_tk(tbma.Text, tbten.Text, tbmk.Text,"HS",
                                tbtenhs.Text, date, cbgioitinh.Text);
                nhanvien.Tao_hs(tbma.Text, cblop.Text, DateTime.Now.Year);
